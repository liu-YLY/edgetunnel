...
;(async () => {
  ...
  for (const 标记 of ['id="o-pref-api"', 'id="o-pref-port"', 'id="btn-verify-api"', 'id="btn-api-to-add"', 'id="o-api-result"', 'id="o-api-out"', 'id="o-lib-random"', 'id="o-lib-count"', 'id="o-lib-port"', 'id="btn-save-lib"', 'id="o-lib-note"', 'id="o-add-test-note"']) {
    assert.ok(html.includes(标记), `HTML 应包含优选元素 ${标记}`);
  }
  assert.ok(html.includes('批量测速并排序'), 'ADD.txt 测速按钮文案应为批量测速并排序');
  // 写入 ADD.txt 只改编辑器内容，不得顺手写 KV（保存仍须用户显式点「保存优选 IP」）
  assert.ok(!/btn-api-to-add[\s\S]{0,600}?\/admin\/ADD\.txt'?, *\{ *method/.test(html), '写入 ADD.txt 不得直接提交 KV');
