...
(async()=>{
 ...
 mf = new Miniflare(convertV4MiniflareOptions({ cf:false, workers:[{ name:"test", modules:true,scriptPath:path.join(__dirname,'../_worker.js'), compatibilityDate:'2025-11-04',compatibilityFlags:['nodejs_als'],kvNamespaces:['KV'], bindings:{ADMIN:'runtime-test-password',KEY:'runtime-test-key',UUID:uuid,OFF_LOG:'true'} }] }));
  ...
  const badSource = await call('/admin/getADDAPI?url=' + encodeURIComponent('ftp://example.com/private?token=hidden'), { headers: { Cookie: cookie } });
  assert.equal(badSource.status, 200);
  const badSourceBody = await badSource.json();
  assert.equal(badSourceBody.success, false);
  assert.equal(badSourceBody.sources[0].state, 'invalid_url');
  assert.ok(!JSON.stringify(badSourceBody).includes('hidden'), '来源诊断不得回显 token');
  assert.equal((await call('/admin/api/link-preview',{method:'POST',body:'{}'})).status,302,'链接预览需要登录');
  const preview=await call('/admin/api/link-preview',{method:'POST',headers:{Cookie:cookie,'Content-Type':'application/json'},body:JSON.stringify({地址:'node.example',端口:8443,路径:'/custom',备注:'测试节点'})});
  assert.equal(preview.status,200,'链接预览应可用');
  const previewLink=(await preview.json()).link;
  assert.ok(previewLink.includes('@node.example:8443?')&&previewLink.includes('path=%2Fcustom')&&previewLink.endsWith('#%E6%B5%8B%E8%AF%95%E8%8A%82%E7%82%B9'),'预览应包含指定地址、端口、路径和备注');
  assert.equal((await call('/admin/api/link-preview',{method:'POST',headers:{Cookie:cookie,'Content-Type':'application/json'},body:'{"地址":"bad/host"}'})).status,400,'非法地址应拒绝');
  assert.equal((await call('/admin/api/link-preview',{method:'POST',headers:{Cookie:cookie,'Content-Type':'application/json'},body:'{"地址":"999.999.999.999"}'})).status,400,'非法 IPv4 不得当作域名通过');
  const trojanGrpc=await call('/admin/api/link-preview',{method:'POST',headers:{Cookie:cookie,'Content-Type':'application/json'},body:'{"协议类型":"trojan","传输协议":"grpc"}'});
  assert.equal(trojanGrpc.status,200,'Trojan gRPC 组合应能生成');
  assert.ok((await trojanGrpc.json()).link.includes('type=grpc&mode=gun&authority='),'Trojan gRPC 链接参数应正确');
  const vlessXhttp=await call('/admin/api/link-preview',{method:'POST',headers:{Cookie:cookie,'Content-Type':'application/json'},body:'{"协议类型":"vless","传输协议":"xhttp"}'});
  assert.equal(vlessXhttp.status,200,'VLESS XHTTP 组合应能生成');
  assert.ok((await vlessXhttp.json()).link.includes('type=xhttp&mode=stream-one&extra='),'VLESS XHTTP 链接参数应正确');
  const ssWs=await call('/admin/api/link-preview',{method:'POST',headers:{Cookie:cookie,'Content-Type':'application/json'},body:'{"协议类型":"ss","传输协议":"ws"}'});
  assert.equal(ssWs.status,200,'SS WebSocket 组合应能生成');
  assert.ok((await ssWs.json()).link.startsWith('ss://'),'SS 链接应使用 ss scheme');
  assert.equal((await call('/admin/api/link-preview',{method:'POST',headers:{Cookie:cookie,'Content-Type':'application/json'},body:'{"协议类型":"ss","传输协议":"grpc"}'})).status,400,'SS gRPC 组合应拒绝');
  const ipv6Preview=await call('/admin/api/link-preview',{method:'POST',headers:{Cookie:cookie,'Content-Type':'application/json'},body:'{"地址":"[2606:4700::1]"}'});
  assert.equal(ipv6Preview.status,200,'合法 IPv6 应通过');
  assert.ok((await ipv6Preview.json()).link.includes('@[2606:4700::1]:443?'),'IPv6 链接应保留方括号');
  assert.equal((await call('/admin/api/link-preview',{method:'POST',headers:{Cookie:cookie,Origin:'https://evil.example'},body:'{}'})).status,403,'跨域写请求应拒绝');
  assert.equal(await (await mf.getKVNamespace('KV')).get('cfg:runtime.example'),null,'链接预览不能写入配置');
  await (await mf.getKVNamespace('KV')).put('ADD.txt','1.1.1.1:443#甲\n1.1.1.1:443#重复\n[2606:4700::1]:8443#乙\nsub://api.example#跳过\nbad/host');
  assert.equal((await call('/admin/api/link-batch',{method:'POST',body:'{"source":"add"}'})).status,302,'批量链接需要登录');
  const batchResponse=await call('/admin/api/link-batch',{method:'POST',headers:{Cookie:cookie,'Content-Type':'application/json'},body:'{"source":"add","协议类型":"vless","传输协议":"ws"}'});
  assert.equal(batchResponse.status,200,'ADD.txt 批量生成应可用');
  const batch=await batchResponse.json();
  assert.equal(batch.节点.length,2,'应保留去重后的两个合法地址');
  assert.equal(batch.重复,1,'应报告重复节点');
  assert.equal(batch.跳过,2,'应报告不支持的指令和非法地址');
  assert.equal(batch.节点[0].来源,'add','结果应标明来源');
  assert.ok(batch.节点[0].链接.endsWith('#%E7%94%B2'),'应保留 ADD.txt 备注');
  assert.equal((await call('/admin/api/link-batch',{method:'POST',headers:{Cookie:cookie,'Content-Type':'application/json'},body:'{"source":"other"}'})).status,400,'未知来源应拒绝');
  ...
  assert.equal(apiBad.status,400,'非法 URL 是参数错误，应返回 400');
  assert.match((await apiBad.json()).msg,/来源地址无效/,'失败响应应说明原因');
  ...
  await (await mf.getKVNamespace('KV')).put('ADD.txt','1.1.1.1:443#plain\nvless://'+uuid+'@node.example:443?type=ws#external');
  const mixedBatch = await call('/admin/api/link-batch',{method:'POST',headers:{Cookie:cookie,'Content-Type':'application/json'},body:'{"source":"preferred"}'});
  const mixedBatchBody = await mixedBatch.json();
  assert.equal(mixedBatchBody.节点.length, 1);
  assert.equal(mixedBatchBody.跳过, 1, '完整外部链接不转换但必须计入跳过数');
  const badPreferredConfig = await call('/admin/config',{method:'POST',headers:{Cookie:cookie,'Content-Type':'application/json'},body:JSON.stringify({优选订阅生成:{local:false,SUB:'sub://bad host'}})});
  assert.equal(badPreferredConfig.status,200);
  const failedBatch = await call('/admin/api/link-batch',{method:'POST',headers:{Cookie:cookie,'Content-Type':'application/json'},body:'{"source":"preferred"}'});
  const failedBatchBody = await failedBatch.json();
  assert.equal(failedBatchBody.节点.length,0);
  assert.equal(failedBatchBody.来源诊断[0].state,'invalid_url');
  for (const target of ['mixed','clash']) {
    const unavailable = await call('/sub?token='+subToken+'&target='+target);
    assert.equal(unavailable.status,502,'来源全部失败不应返回空成功订阅或回落转换器');
    assert.match(await unavailable.text(),/来源诊断/);
  }
  console.log('[PASS] workerd 登录/鉴权/配置校验/KV 写入/跨站拒绝/来源诊断/分帧 VLESS/Trojan/SS WebSocket→原生 TCP→回传');
