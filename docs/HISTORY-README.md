# 历史 README 快照（2026-09-16 修复前）

> 仅供追溯。默认出口、并发容量、KV 同步时间及部署步骤已被 CF-DEPLOYMENT.md 替代，不作为当前运行保证。
...
5. 配置管理页（可选）：
   - 登录后访问 `/admin/config` 新增「配置」页，可直接编辑 **KV 全量配置**并保存到 `cfg:{host}`（以访问域名分桶），保存即生效，无需改环境变量/重新部署；
   - 该页同时只读展示当前环境变量（env）配置；配置读取优先级为 `path 参数 > KV(cfg:{host}) > env > 默认值`。
...
> ### ⚠️ M2-P0 行为变更（出站默认官方直连）
> 自本版本起，**未设置 `PROXYIP` 的部署默认不再请求第三方 `{colo}.SsSs.nEt` 反代域名**，
> 改为内置 10 个 Cloudflare 官方地址（多候选打乱竞速 + 失败回落直连），运行时零外部依赖。
> 想保留旧的"按地区挑出口"行为：设置环境变量 `出站模式=region`，并在节点 path 上写 `wk=地区`。
> 已配置 `PROXYIP` 的部署不受影响（自动进入 manual 模式，与升级前逐字一致）。
...
| **PROXYIP** | ❌ | `proxyip.cmliussss.net:443` | 全局自定义反代 IP；设置后出站模式自动变为 **manual**（随机取一 + 关闭直连兜底） |
| **出站模式** / **EGRESS_MODE** | ❌ | `region` | 出站方式三选一：`auto`（默认，内置官方地址直连）/ `region`（path `wk` 指定地区走地区反代模板）/ `manual`（由 `PROXYIP` 触发） |
...
配置读取优先级为 **`path 参数 > KV(cfg:{host}) > env > 默认值`**：

- 登录后台后访问 `/admin/config`「配置」页，可将全量配置 JSON 直接保存到 KV 键 `cfg:{host}`（`{host}` 为访问域名，多域名互不覆盖），保存即生效，无需改环境变量或重新部署。
- 读取时若 `cfg:{host}` 存在则整体覆盖 env/默认值；缺失时自动回退 env/默认值（缺键不崩溃）。
- 与旧 KV 键 `config.json` 保持兼容、互不冲突；仅使用既有 `KV` 绑定，不新增任何环境变量或绑定；存量「仅配 ADMIN+KV」的部署行为不回退。

### 🛣 path 逐节点覆盖（连接级 4 参数）

在 WS 握手链路的反代上下文构建处解析 URL 查询参数，**仅白名单内的 4 个参数生效，其余一律忽略**：

| 参数 | 含义 |
| :--- | :--- |
| `p` | 指定连接级 ProxyIP，直接在路径上覆盖该连接的反代 IP；**写 `p` 则地区匹配整体跳过** |
| `wk` | 指定地区（如 `hk`/`us`），**端到端生效**：`出站模式=region` 时按 `wk` 生成地区反代模板（`{wk}.…SsSs.nEt`）并保留直连兜底；`auto` 模式下忽略（官方直连不依赖地区域名）；与 `p` 互斥（写 `p` 时忽略 `wk`） |
| `rm` | 地区匹配开关：`rm=no` 强制关闭（即使 `region` 模式 + `wk` 也不走地区模板）；缺省视为开启 |
| `s` | 指定该连接的出站代理（`socks5://…` 等链式代理，或代理地址） |

示例：

```url
/?p=proxyip.example.com:443            # 该连接使用指定 ProxyIP（端到端生效）
/?p=1.2.3.4:443&wk=hk                  # p 生效，wk 被忽略（p 与 wk 互斥）
/?s=socks5://user:pass@127.0.0.1:1080  # 该连接走指定出站代理（端到端生效）
/?wk=hk                                # 出站模式=region 时该连接走 HK 地区反代模板；auto 模式下忽略
/?wk=hk&rm=no                          # rm=no 强制关闭地区匹配（即使 region 模式）
```
